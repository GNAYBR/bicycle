    ...
    <div class="public-content">
        ...
        <div class="public-content-cont">
            ...
            <div class="page">
                ...
                <c:if test="${pageBean.currentPage!=1}"><a href="/syuser/find.do?currentPage=${pageBean.currentPage-1}&userName=${userName}&roleId=${roleId}"><</a></c:if>
                <c:forEach begin="1" end="${pageBean.totalPage}" step="1" varStatus="s">
                    <c:if test="${s.index == pageBean.currentPage}">${s.index}</c:if>
                    <c:if test="${s.index != pageBean.currentPage}"><a href="/syuser/find.do?currentPage=${s.index}&userName=${userName}&roleId=${roleId}">${s.index}</a></c:if>
                </c:forEach>
                <c:if test="${pageBean.currentPage==pageBean.totalPage}">></c:if>
                <c:if test="${pageBean.currentPage!=pageBean.totalPage}"><a href="/syuser/find.do?currentPage=${pageBean.currentPage+1}&userName=${userName}&roleId=${roleId}">></a>
                </c:if>
            </div>
